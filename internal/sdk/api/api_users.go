...
// Maximum number of items to return per page. If not provided, all items are returned. When provided, enables pagination and the response will include next_token for retrieving subsequent pages. Valid range: 1-100.
func (r ApiUsersGetRequest) Limit(limit int32) ApiUsersGetRequest {
	r.limit = &limit
	return r
}

// Cursor token for retrieving the next page of results. Obtained from the next_token field of a previous paginated response. Only valid when the limit parameter is also provided.
...
// Whether to include the groups array in the response.
