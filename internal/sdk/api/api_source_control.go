...
Deploys the latest changes from the source control provider to the instance. You can use the GET /deployment/{id} endpoint to check the status of the deployment. The API token must have the "Source Control > Deploy" scope.
...
Get the status of a deployment. The API token must have the "Source Control > Deploy" scope.
...
Returns a list of source control deployments in reverse chronological order. This returns the same data as shown in the deploy dashboard. The API token must have the "Source Control > Deploy" scope.
...
Attempts a test deployment (no permanent changes) using the provided commit SHA in order to determine whether the commit is valid. The API token must have the "Source Control > Test Deploy" scope.
